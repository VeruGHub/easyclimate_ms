...
## Text results
opts_chunk$set(echo = TRUE, warning = TRUE, message = FALSE)
## Code decoration
...
```{r knitcitations, cache=FALSE, include=FALSE}
...
In this paper we present *easyclimate* a package to process and download climatic data with R [@R_Core_Team_2021] available from the Comprehensive R Archive Network (CRAN; **link**). In the last decades, ecological science and application has experienced an increasing demand of harmonized climatic data at wide spatial scales and spanning long temporal series (e.g. for the study of the effects of climate change on vegetation, [@Ruiz-Benito2020]). In this context, *easyclimate* has been developed to facilitate the downloading and processing of high-resolution (1 km\^2) daily climate data (precipitation, minimum and maximum temperatures) for Europe. Data are currently available from 1950 to 2020 and hosted at [University of Natural Resources and Life Sciences, Vienna, Austria](https://boku.ac.at/en/wabo/waldbau/wir-ueber-uns/daten). This climatic dataset was originally built by Moreno & Hasenauer [@moreno2016spatial] and further developed by W. Rammer, C. Pucher & M. Neumann. It is a downscaled dataset based on [WorldCllim](https://worldclim.org/data/worldclim21.html#) and [E-Obs](https://surfobs.climate.copernicus.eu/dataaccess/access_eobs.php#datafiles). For this R package we implemented Cloud-Optimised Geotiffs so that the user can obtain daily climate data for thousands of sites and days within minutes, without having to download huge rasters.
...
The results of the package *easyclimate* can be used directly or with simple calculations as the shown previously as input of other packages to calculate climatic indices (e.g. [climInd](https://cran.r-project.org/web/packages/ClimInd/index.html), [SPEI](https://cran.r-project.org/web/packages/SPEI/index.html)).

# SAVINGS IN TIME

The dataset is accessible to free download as raster layers tiled or for the entire Europe. However, if only climatic information for specific areas is needed, Cloud-Optimised Geotiffs suppose a high reduction in time. In the next example we download daily precipitation for one year in an area of *ca*. 100 km\^2 using the traditional and the *easyclimate* method. There is a reduction in **XX** minutes in the processing time.
...
url <- "ftp://palantir.boku.ac.at/Public/ClimateData/TiledClimateData/B/9/Prcp/2010_B_9.tif"

out <- "C:/Users/veruk/Desktop/Disco/easyclimate/easyclimate_ms/Prcp2010_B_9.tif"

t1.1 <- Sys.time()

try(getURL(url)) #I can not access to the server and I don't know how to do it in R
try(GET(url, authenticate('guest', ""), write_disk(out, overwrite = TRUE)))

raster1 <- terra::rast("C:/Users/veruk/Desktop/Disco/easyclimate/easyclimate_ms/Prcp2010_B_9.tif") #It would be necessary to have a reproducible example not including my specific path
raster1 <- terra::extract(rasters1, coords_poly, xy = TRUE)
...
Additionally, there are big efforts among the governments and the scientific communities for harmonizing data from European national forest inventories (<http://enfin.info>, <https://efi.int/knowledge/models/efiscen/inventory>, <http://project.fundiveurope.eu/>). Those inventories repeated in many countries along decades are usually done with spatial resolutions of few kilometers [@Mauri2017]. The spatial and temporal framework fits with this of the climatic database extracted with *easyclimate*, making this tool spatially useful to answer a myriad of ecological questions related to climatic changes along Europe.
...
```{r write_citations, eval=FALSE, cache=FALSE, include=FALSE}
...
kable(head(prec, n=10), caption = "Daily precipitation (Prcp; mm) for a given point extracted with easyclimate. Only ten first days are shown")
```
...
```{r Fig-1, fig.cap="Maximum temperature for a given polygon in two different days of the year"}
...
terra::plot(ras_tmax, 2, col = rev(heat.colors(20)), type = "continuous", 
            ...
            mar = c(4, 1, 4, 3), main = "August 1 2012")
